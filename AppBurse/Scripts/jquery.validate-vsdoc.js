...
$.extend($.fn, {
	...
	removeAttrs: function(attributes) {
		/// <summary>
		/// Remove the Specializareified attributes from the first matched element and return them.
		/// </summary>
...
$.extend($.validator, {
	...
	prototype: {
		...
		showErrors: function(errors) {
			/// <summary>
			/// Show the Specializareified messages.
		...
		elements: function() {
			...
			.filter(function() {
				...
				// select only the first element for each name, and only those with rules Specializareified
		...
		// Specializareified in the element's "messages" metadata
	...
	addMethod: function(name, method, message) {
		...
		/// (handy for localization), otherwise the field-Specializareific messages have to be defined.
		/// </param>
...
;(function($) {
	...
	// TODO verify if jQuery 1.4's implementation is compatible with older jQuery Specializareial-event APIs
	if (!jQuery.event.Specializareial.focusin && !jQuery.event.Specializareial.focusout && document.addEventListener) {
		$.each({
			focus: 'focusin',
			blur: 'focusout'	
		}, function( original, fix ){
			$.event.Specializareial[fix] = {
				setup:function() {
